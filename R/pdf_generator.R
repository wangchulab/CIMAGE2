...
i <- id
dir.create(paste(output.path,"/PDF",sep=""))
message(paste("The page number drawing is",id,sep=" "))

  out.filename <- paste(output.path,"/PDF/",out.filename.base,"_",id,".pdf",sep="")
...
#	}
  ...
  for ( j in 1:ncross ) {
    ...
    print(c(ms1.scan.rt,rt.window.width,rt.min))
    ...
    xlimit_ylimit <-  xlimit_ylimit <- c(which(xfile@scantime>(rt.min+rt.window.width-30))[1]-1, which(xfile@scantime>(rt.max-rt.window.width+30))[1] )
    #print(c(rt.min,rt.max))
    #print(xlimit_ylimit)
    #print(xlimit)
    ylimit <- range(c(raw.ECI.light[[2]][xlimit_ylimit[1]:xlimit_ylimit[2]], raw.ECI.heavy[[2]][xlimit_ylimit[1]:xlimit_ylimit[2]]))
