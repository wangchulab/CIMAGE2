...
column.names <- c("index","ipi", "description", "symbol", "sequence", "mass", "charge", "segment",
                  integrated.area.ratio, linear.regression.ratio , light.integrated.area, peak.noise.information, linear.regression.R2, "entry","RT","link" )
...
#	}
  ...
  ## itself carries heavy isotopes (C13/N15/H2, e.g. after swapping light/heavy
  ## tables), isotope.dist() prepends leading zeros and shifts the whole envelope
  ## right. mono.mass already accounts for those isotopes, so it corresponds to the
  ## first non-zero index. Measure the most-abundant offset from there, not index 1.
  ## (No-op when the light label is clean, since then i.mono==1.)
  ...
  this.df <- c(i, ipi, description, symbol, peptide, round(mono.mass,digits=4), charge, segment,
               i.ratios,l.ratios ,light.int.v, NP.value, r2.v, entry.index,tag.rt,
